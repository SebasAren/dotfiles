...
Run `mise run pre-commit` before committing — it runs format, lint, typecheck, and tests via `.mise/tasks/pre-commit`. The git pre-commit hook (`.git/hooks/pre-commit`) invokes it automatically on `jj commit`.
