...
- **Pre-commit checks**: run `mise run pre-commit` (format + lint + typecheck + tests). The git pre-commit hook (`.git/hooks/pre-commit`) invokes it automatically on `jj commit`.
