...
import wtWorktreeExtension, { generateBranchName, formatDuration } from "./index";
...
describe("renderResult — collapsed", () => {
	...
	it("shows success icon and branch in collapsed view", () => {
		const result = tool.renderResult(makeResult(), { expanded: false }, makeTheme(), {});
		expect(result.text).toContain("✓");
		expect(result.text).toContain("agent/fix-123");
	});

	it("shows error icon when isError is true", () => {
		const result = tool.renderResult(makeResult({ isError: true }), { expanded: false }, makeTheme(), {});
		expect(result.text).toContain("✗");
	});
...
describe("renderResult — expanded", () => {
		...
		return {
			...
			details: {
				...
				subagentResult: {
					...
					stderr: "",
					usage: { input: 2000, output: 1000, cacheRead: 500, cacheWrite: 0, cost: 0.12, contextTokens: 0, turns: 5 },
					model: "gpt-4o",
				},
				durationMs: 120_000,
				autoMerge: true,
	...
	it("returns a Container with children", () => {
		const result = tool.renderResult(makeResult(), { expanded: true }, makeTheme(), {});
