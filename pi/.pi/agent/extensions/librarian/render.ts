...
export function renderResult(
	...
	return renderSubagentResult({
		...
		buildExpandedHeader: (d, t) => {
			...
			if ((d as any).library) parts.push(t.fg("muted", ` · Library: `) + t.fg("accent", String((d as any).library)));
			if ((d as any).focus) parts.push(t.fg("muted", ` · Focus: `) + t.fg("dim", String((d as any).focus)));
			return parts.join("");
		},
	});
}
