...
// Module-level guard against double-registration in subagent contexts.
// The main agent loads this extension first (sets loaded=true). Subagent
// sessions (explore, librarian) attempt to load it again — the guard
// prevents registering handlers on the subagent's ExtensionRunner, which
// would send false working/idle transitions to herdr's socket.
// On session_shutdown (reload), the flag resets so the new ExtensionRunner
// can re-register handlers cleanly.
let loaded = false;
...
export default function (pi) {
  ...
  // Reset the loaded flag on shutdown so reloads can re-register.
  // Must be set before the cleanup handler so it runs first.
