 ...
 */
export function reuseOrCreateText(context: {
...
export function renderSubagentResult(
	...
	const output =
		textContent?.type === "text" ? textContent.text ?? "(no output)" : "(no output)";
	...
	if (expanded) {
		...
		container.addChild(
			new Text(
				`${icon} ${theme.fg("toolTitle", theme.bold(toolName))}`,
				0,
				0,
			),
		);
		...
		if (isError) {
			...
			for (const section of sections) {
				...
				if (section.content) {
					container.addChild(
						new Markdown(section.content, 0, 0, mdTheme),
					);
				}
			}
		} else {
