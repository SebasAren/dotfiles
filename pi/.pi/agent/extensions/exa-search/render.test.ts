...
describe("renderFetchResult", () => {
		...
		const result = renderFetchResult(
			{
				...
				details: { urls: ["url1", "url2"], format: "text", successCount: 2, errorCount: 0, truncated: false },
			},
			{ expanded: false, isPartial: false },
			makeTheme(),
		);
	...
	it("shows error count", () => {
		const result = renderFetchResult(
			{
				content: [{ type: "text", text: "Content" }],
				details: { urls: ["url1", "url2"], format: "text", successCount: 1, errorCount: 1, truncated: false },
			},
			{ expanded: false, isPartial: false },
			makeTheme(),
		);
	...
	it("shows error icon when all failed", () => {
		const result = renderFetchResult(
			{
				content: [{ type: "text", text: "Content" }],
				details: { urls: ["url1"], format: "text", successCount: 0, errorCount: 1, truncated: false },
			},
			{ expanded: false, isPartial: false },
			makeTheme(),
		);
